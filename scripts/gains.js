function calculateGainPerKWh(application, inputData) {
    ...
    if (application.capexScenario === '既存装置との価格差') {
      capexReductionGainPerKWh =
        (application.existingEquipmentCost - application.newEquipmentCost) /
        systemCapacity;
    }
    ...
    if (application.opexScenario === '電気代と既存エネルギー代の価格差') {
      ...
      operationGainPerKWh =
        ((fuelPricePerLiter * annualFuelConsumption -
          retailPowerPrice * electricityConsumption) *
          yearsOfUse) /
        systemCapacity;
    } else if (
      application.opexScenario === '余剰電力と日中最高電力価格との価格差'
    ) {
      operationGainPerKWh =
        inputData.sunnyDaysRatio *
        application.averageEquivalentCyclesPerDay *
        365 *
  ...
  function calculateSohDeterioration(application) {
    ...
    if (application.opexScenario === '電気代と既存エネルギー代の価格差') {
      const annualElectricityConsumption =
        application.annualMileage / application.electricEfficiency;
    ...
    const sohDeterioration =
      (inputData.CalendarDegradationFactor * days +
        inputData.CycleDegradationFactor * equivalentCycles) *
      0.001; // 係数を1000倍しているので割り戻す
